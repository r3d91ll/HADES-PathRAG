#!/usr/bin/env python3
"""
Test driver for the document processing, chunking, and embedding pipeline.

This script processes text files through the document processing and chunking
modules to generate a JSON object for embedding module testing.
"""
...
def process_and_chunk_document(file_path: str) -> Dict[str, Any]:
    ...
    document = {
        ...
        "type": processed_doc.get("format", Path(file_path).suffix[1:])
    }
    ...
    schema_doc = {
        ...
        "content": processed_doc.get("content", ""),
        "source": file_path,
        "document_type": processed_doc.get("format", Path(file_path).suffix[1:]),
